...
std::vector<double> solve_dup(double eps, double tau, int n,
   ...
   while (rel_res >= eps) {
       ...
       MPI_Allgatherv(&x[offsets[rank]], counts[rank], MPI_DOUBLE,
                      x_new.data(), counts.data(), offsets.data(),
...
std::vector<double> solve_dist(double eps, double tau, int n,
   ...
   while (rel_res >= eps) {
       ...
       // Умножение с пересылками по кольцу
       ...
       // Вычисление невязки и обновление
   ...
   MPI_Allgatherv(x_local.data(), counts[rank], MPI_DOUBLE,
                  x_global.data(), counts.data(), offsets.data(),
                  MPI_DOUBLE, MPI_COMM_WORLD);
   return x_global;
}
...
int main(int argc, char* argv[]) {
   ...
   // Глобальная правая часть (для режима 0)
   ...
   if (rank == 0) {
       ...
       // Проверка, что требуемая точность достигнута
